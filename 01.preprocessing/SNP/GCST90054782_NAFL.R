...
#
summary = fread("GCST90054782_buildGRCh37.tsv")
summary = data.frame(summary)
title = "GCST90054782_NAFL"
#
#source("c:/Dropbox/ddasic/bi/SNP_density_plot.R")
#
#summary$base_pair_location
...
#View(summary[1:100, ])
summary$effect = log(summary$odds_ratio)
summary$lo = log(summary$ci_lower)
summary$se1 = summary$effect - summary$lo
summary$stderr = summary$se1/1.96
summary$p = summary$p_value
summary = summary[, c("marker", "effect", "stderr", "p", "chr", "pos")]
write.table(summary, paste0(title, "_proc.txt"), sep = "\t", row.names = F, quote = F)
