...
#
phenotype = sample$group
table(phenotype)
pheno_color = c("darkolivegreen1", "gold3", "darkred")
pheno_color = c("darkolivegreen1", "darkred", "grey")
pheno_type = c("CN", "NASH", "Others")

#
idx.c = grep("healthy", phenotype)
idx.d1 = grep("Non alcoholic steatohepatitis", phenotype)
phenotype[] = pheno_type[3]
...
cut_off = quantile(rowSds, probs = 0.7)
