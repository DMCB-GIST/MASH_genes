...
#
sample$id = gsub("human liver sample ", "Sample_", sample$id)
...
pheno_color = c("darkolivegreen1", "gold", "tomato", "darkred")
pheno_type = c("CN", "F0F1", "F2", "F3F4")

#
idx.c = grep("Control", phenotype)
idx.d1 = grep("F0F1", phenotype)
idx.d2 = grep("F2", phenotype)
idx.d3 = grep("F3", phenotype)
phenotype[idx.c] = pheno_type[1]
phenotype[idx.d1] = pheno_type[2]
phenotype[idx.d2] = pheno_type[3]
phenotype[idx.d3] = pheno_type[4]
...
figure_storage = paste0("F.heatmap_", id_expr, "_mm.pdf")
